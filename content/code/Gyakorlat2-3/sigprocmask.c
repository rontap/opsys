...
int main() {
    ...
    if (child > 0) {
        ...
        printf("[parent] The blocking of SIGTERM %i signal is released, so the formerly sent SIGTERM signal arrives\n",
               SIGTERM);
        ...
        printf("[child] Waits 3 seconds, then send a SIGTERM %i signal (it is blocked)\n", SIGTERM);
        sleep(3);
        kill(getppid(), SIGTERM);
        printf("[child] Waits 3 seconds, then send a SIGUSR1 %i signal (it is not blocked)\n", SIGUSR1);
        sleep(3);
